...
public class Code {
    ...
    public static String code() {
        ...
        SymbolTable.classes().forEach(c -> {
        	...
        	println("CREATE $2 " + vmt.size());
        	println("SETWORD $7 " + c.index + " $2");
        	vmt.forEach(f -> {
        		String label = c.name + "::" + f.name;
	        	println("SETWORD $2 " +  f.index + " " + "\"" + label + "\"");
        	});
        });
        newline();
    ...
    public static void call(String function, List<String> arguments) {
        // Store framepointer
        push("$FP");
        ...
        println("CALL [$SP] " + function);

        // Clear return address and arguments
        println("SUBI $SP $SP " + (1 + arguments.size()));
    ...
    public static void code(Class c) {
    	c.methods.values().forEach(f -> code(f));
    }

    public static void code(Function f) {
    	// Reset static fields
    	labelTemplate = "";
    	if(f.contextClass != null) {
    		labelTemplate = f.contextClass.name;
    	}    
		labelTemplate += "::"+ f.name;
    	labelOrder = 0;

    	// Skip ::print
    	if(f.body == null && f.name.equals("print")) {
			return;
    	}
        ...
        println("ADDI $SP $SP " + f.variableIndex);
        newline();
    	...
    	switch(s.option) {
    		case DECLARATION:
    			...
    			if(v.type == Type.STRING) {
    				println(String.format("SET [$FP%+d] \"\"", v.index));
    			} else {
    				println(String.format("SET [$FP%+d] 0", v.index));
    			}
    			break;
    		case ASSIGNMENT:
    			code(s.ex);
    			if(s.path.path == null) {
    				...
    				println(String.format("SET $1 [$FP%+d]", s.path.handle.index));
    				for(int index = 0; index < path.size()-1; index++) {
	    				println("GETWORD $1 $1 " + (path.get(index).index+1));
    				}
    				println("SETWORD $1 " + (path.get(path.size()-1).index+1) + " [$SP]");
    	...
    	if(ex.option == Expression.Option.PATH) {
    		...
    		if(path.path == null) {
				...
				if(ex.path.type == Type.STRING) {
                    println(String.format("COPY $1 [$FP%+d]", handle.index));
                    push("$1");
                } else {
                    push(handle);
                }
				...
				println(String.format("SET $1 [$FP%+d]", handle.index));
				for(int index = 0; index < list.size(); index++) {
    				println("GETWORD $1 $1 " + (list.get(index).index+1));
				}
    ...
    public static void push(Variable src) {
    	push(String.format("[$FP%+d]", src.index));
    }
    ...
    public static void pop(Variable dst) {
    	pop(String.format("[$FP%+d]", dst.index));
    }
